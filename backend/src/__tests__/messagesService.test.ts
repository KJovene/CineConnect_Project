...
describe('messagesService', () => {
  ...
  it('createMessage insere et retourne le message cree', async () => {
    const returning = jest.fn().mockResolvedValue([{ message_id: 7, content: 'hello' }]);
  ...
  it('getRecentConversations deduplique par interlocuteur', async () => {
    const where = jest.fn().mockReturnValue({
      orderBy: jest.fn().mockResolvedValue([
        { message_id: 5, sender_id: 1, receiver_id: 2, content: 'last with 2', sent_at: new Date() },
        { message_id: 4, sender_id: 2, receiver_id: 1, content: 'older with 2', sent_at: new Date() },
        { message_id: 3, sender_id: 3, receiver_id: 1, content: 'with 3', sent_at: new Date() },
      ]),
    });
