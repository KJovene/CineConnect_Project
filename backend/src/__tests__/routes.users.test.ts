...
describe('users routes', () => {
  ...
  it('GET /me/comment-replies retourne les notifications', async () => {
    mockReviewsService.getUserCommentReplyNotifications.mockResolvedValue([{ replyReviewId: 1 }]);
  ...
  it('GET /me/comment-replies retourne 500 en erreur', async () => {
    mockReviewsService.getUserCommentReplyNotifications.mockRejectedValue(new Error('x'));
  ...
  it('GET / retourne [] si seulement soi-meme', async () => {
    mockDb.select.mockReturnValue({
      from: jest.fn().mockReturnValue({
        where: jest.fn().mockReturnValue({ limit: jest.fn().mockResolvedValue([{ id: 1, name: 'Me', email: 'm', image: null }]) }),
      }),
    });
