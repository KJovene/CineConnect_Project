...
describe('films routes', () => {
  ...
  it('GET /search retourne data', async () => {
    mockFilmsService.searchFilms.mockResolvedValue({ results: [], totalResults: 0, page: 1 });
  ...
  it('GET /community-reviews retourne 500', async () => {
    mockReviewsService.getLatestCommunityReviews.mockRejectedValue(new Error('x'));
  ...
  it('GET /:omdbId retourne film enrichi', async () => {
    ...
    mockReviewsService.getFilmRatingSummary.mockResolvedValue({ averageRating: 4, totalRatings: 2, userRating: 5 });
