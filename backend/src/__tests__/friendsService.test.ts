...
describe('friendsService', () => {
  ...
  it('getFriends enrichit avec friend mappe', async () => {
    const firstWhere = jest.fn().mockResolvedValue([
      { friend_id: 1, user_id: 1, friend_user_id: 2, status: 'accepted', created_at: new Date() },
    ]);
    ...
    expect(result[0].friend).toEqual({ id: 2, name: 'Bob', email: 'b@b.com', image: null });
  });

  it('getFriends met friend a null si utilisateur introuvable', async () => {
    const firstWhere = jest.fn().mockResolvedValue([
      { friend_id: 1, user_id: 2, friend_user_id: 1, status: 'accepted', created_at: new Date() },
    ]);
    const secondWhere = jest.fn().mockResolvedValue([]);

    mockDb.select
      .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: firstWhere }) })
      .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: secondWhere }) });
  ...
  it('getPendingRequests retourne [] quand aucune demande', async () => {
    const where = jest.fn().mockResolvedValue([]);
    mockDb.select.mockReturnValue({ from: jest.fn().mockReturnValue({ where }) });
  ...
  it('getPendingRequests enrichit avec requester', async () => {
    const firstWhere = jest.fn().mockResolvedValue([
      { friend_id: 10, user_id: 2, friend_user_id: 1, status: 'pending', created_at: new Date() },
    ]);
    const secondWhere = jest.fn().mockResolvedValue([{ id: 2, name: 'Bob', email: 'b@b.com', image: null }]);
  ...
  it('sendFriendRequest refuse auto-ajout', async () => {
    await expect(sendFriendRequest(1, 1)).rejects.toThrow("Impossible de s'ajouter soi-même");
  });
  ...
  it('sendFriendRequest refuse relation existante', async () => {
    ...
    await expect(sendFriendRequest(1, 2)).rejects.toThrow('Relation déjà existante');
  });
  ...
  it('sendFriendRequest cree une relation pending', async () => {
    ...
    const returning = jest.fn().mockResolvedValue([{ friend_id: 1, user_id: 1, friend_user_id: 2, status: 'pending' }]);
  ...
  it('acceptFriendRequest throw si demande absente', async () => {
    ...
    await expect(acceptFriendRequest(1, 2)).rejects.toThrow('Demande introuvable');
  });

  it('acceptFriendRequest retourne resultat', async () => {
    mockDb.update.mockReturnValue({
      set: jest.fn().mockReturnValue({
        where: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([{ status: 'accepted' }]) }),
      }),
    });
  ...
  it('rejectFriendRequest throw si demande absente', async () => {
    ...
    await expect(rejectFriendRequest(1, 2)).rejects.toThrow('Demande introuvable');
  });

  it('rejectFriendRequest retourne resultat', async () => {
    mockDb.update.mockReturnValue({
      set: jest.fn().mockReturnValue({
        where: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([{ status: 'rejected' }]) }),
      }),
    });
