...
describe("messages routes", () => {
  ...
  it("GET /conversations retourne liste", async () => {
    mockMessagesService.getRecentConversations.mockResolvedValue([
      { message_id: 1 },
    ]);
  ...
  it("GET /conversations retourne 500 en erreur", async () => {
    mockMessagesService.getRecentConversations.mockRejectedValue(
      new Error("x"),
    );
  ...
  it("GET /incoming retourne notifications", async () => {
    mockMessagesService.getIncomingMessages.mockResolvedValue([
      { message_id: 1 },
    ]);
