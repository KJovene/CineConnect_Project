...
export function useSignup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = registerRequestSchema.safeParse({ email, username: normalizedName, password });
    if (!validation.success) {
      const field = validation.error.issues[0]?.path[0];
      if (field === "email") setError("Adresse email invalide.");
      else if (field === "username") setError("Le pseudo doit contenir au moins 2 caractères.");
      else if (field === "password") setError("Le mot de passe doit contenir au moins 8 caractères.");
