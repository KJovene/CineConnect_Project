...
const SIDEBAR_SECTIONS: SidebarNavSection[] = [
  ...
  },
  {
    title: 'Social',
    items: [
      { id: 'community', icon: <HiOutlineUsers size={20} />, label: 'Profil', href: '/profil', isActive: true },
      { id: 'discussions', icon: <HiOutlineChatBubbleLeftRight size={20} />, label: 'Discussions', href: '/discussion' },
    ],
  },
];
      ...
      <div className="px-8 pt-28 pb-8 max-w-3xl mx-auto">
        ...
        <section className="mb-10">
          <div className="flex items-center gap-5 p-6 bg-[#0A0A0A] border border-white/5 rounded-2xl">
            ...
            />
            <div>
              <h1 className="text-xl font-bold text-white">{currentUser?.name ?? 'Utilisateur'}</h1>
        ...
        <section className="mb-8">
          <h2 className="text-lg font-semibold text-white mb-4">Ajouter un ami</h2>
          ...
          {search.length >= 2 && (
            <div className="space-y-2">
              {searchResults.length === 0 ? (
                <p className="text-sm text-neutral-600 text-center py-4">Aucun résultat</p>
              ) : (
                searchResults.map((u: UserSearchResult) => (
                  <SearchUserRow
                    ...
                    onAdd={(id) => {
                      ...
                      sendRequest.mutate(id, { onError: (err) => setRequestError(err.message) });
                    }}
